...
osm2pgsql_OPTS="--slim -d ${OSM_DB} -C 400 --cache-strategy sparse --number-processes ${NP} --hstore"
...
function style_osm_bright(){
	echo "###############################	$(date)	style_osm_bright() started"
	...
	echo "########################################### 10 Compiling the stylesheet"
	...
	echo "###############################	$(date)	style_osm_bright() finished"
}
...
function style_osm_carto(){
	...
	echo "###############################	$(date)	style_osm_carto() finished"
}
...
echo "########################################### 1 Update ATP and isntall needed packages"
apt-get clean
apt-get update
...
echo "########################################### 5 Installing osm2pgsql and mapnik"
...
echo "########################################### 7 Install modtile and renderd"
mkdir -p ~/src
...
echo "########################################### 8 Stylesheet configuration"
...
echo "########################################### 11 Setting up your webserver"
...
echo "########################################### Set Leaflet point of view"
...
echo "########################################### Turn off autovacuum and fsync during load of PBF"
...
echo "########################################### 13. Loading data into your server"
